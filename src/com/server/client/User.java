...
import com.server.command.CommandParser;
...
public class User implements Runnable
{
	...
	public void run()
	{
		...
		while (running)
		{
			...
			if (GsonParser.IsInJsonFormat(readLine))
			{

				SessionManager.record(readLine);

				Event parsedEvent = GsonParser.GetEvent(readLine);
				readLine = "";

				StateManager.AddEventReceived(parsedEvent);
			}
			// CVE format
			else
			{
				...
				if (command != null)
				{
					...
					else if (command.getType() == Command.CommandType.TELL)
					{
						...
						if (args.size() >= 2)
						{
							args.set(1, userID + "*whispers*:" + args.get(1));
						}
						command = new Command(command.getType(), args);

						server.queueCommand(command);
					}
					else if (command.getType() == Command.CommandType.USERS)
					{
						ArrayList<String> args = command.getArguments();
						args.add(userID);
						server.queueCommand(command);
			...
			if (command != null && command.getType() == Command.CommandType.LOGIN)
			{
				...
				if (command.getArguments().size() > 0)
				{
					...
					if (server.registerUser(userID, this))
					{
						...
						response += " Welcome " + userID + " to the CVE ChatServer , Type a command to begin.";
