...
public class CommandParser
{
	public static Command parseLine(String line)
	{
		...
		else if (commandString.equals(Command.CommandType.USERS.matchedString))
		{
			command = handleUsersCommand(commandString, arguments);
		}
	...
	private static Command handleUsersCommand(String commandString, ArrayList<String> arguments)
	{
		Command command = null;
		...
		{
			// give a warning, but we can still proceed
			System.out.println("users: additional arguments ignored");
		}
		// clear the args since we dont need them
		arguments.clear();

		return new Command(Command.CommandType.USERS, arguments);
	}
}
